...
- **Custom Functions & Abbreviations**:  
  ...
  - **`which` function**: Wraps `command -s` and outputs a split hyperlink. The directory part and the filename part are separate clickable links (e.g., clicking the path opens the folder, clicking the filename opens the file in a supported terminal).
