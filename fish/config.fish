...
if status is-interactive
    ...
    function cd; if set -q argv[1]; __zoxide_z $argv; else; if test -s /tmp/fzf-history-$USER/universal-last-dirs-$fish_pid; set -l t (cat /tmp/fzf-history-$USER/universal-last-dirs-$fish_pid | fzf --height 40% --reverse --header="Select path"); if test -n "$t"; if test -d "$t"; __zoxide_z "$t"; else; __zoxide_z (dirname -- "$t"); end; else; __zoxide_z ~; end; else; __zoxide_z ~; end; end; end
    function cdi; __zoxide_zi $argv; end
    function nano; if set -q argv[1]; command nano $argv; else; set -l t (test -s /tmp/fzf-history-$USER/universal-last-files-$fish_pid; and cat /tmp/fzf-history-$USER/universal-last-files-$fish_pid | fzf --height 40% --reverse --header="Select file"); test -n "$t"; and command nano "$t"; end; end
