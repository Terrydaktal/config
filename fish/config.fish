...
if status is-interactive
    ...
    function which; for p in (command -s $argv); if test -L $p; set -l t (realpath $p); twig --color always -LxXUF $p; set -l l_meta (script -qfc "twig -psot -L '$p'" /dev/null | tr -d '\r'); set -l t_meta (script -qfc "twig -psot -L '$t'" /dev/null | tr -d '\r'); echo "$l_meta -> $t_meta"; else; twig --color always -psot -XUF -L $p; end; end; end
